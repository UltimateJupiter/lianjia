    ...
    for page in range(total_pages):
        ...
        for name in nameList:  # per house loop
            ...
            try:
                ...
                info = houseaddr.get_text().decode('unicode_escape').split('|')
                ...
                detail = housefloor.get_text().decode('unicode_escape').split('|')
                info_dict.update({u'years': detail[-1].strip()})

                community = name.find("a", {"class": "laisuzhou"})
                info_dict.update(
                    {u'community': community.span.get_text().decode('unicode_escape').strip()})
                info_dict.update({u'followInfo': ''})

                tax = name.find("div", {"class": "property-tag-container"})
                info_dict.update({u'taxtype': "".join(tax.get_text().decode('unicode_escape').split())})

                totalPrice = name.find(
                    "span", {"class": "total-price strong-num"})
                info_dict.update(
                    {u'totalPrice': totalPrice.get_text().decode('unicode_escape').strip()})

                unitPrice = name.find(
                    "span", {"class": "info-col price-item minor"})
                info_dict.update({u'unitPrice': unitPrice.get_text().decode('unicode_escape').strip()})
            except:
                continue
    ...
    for page in range(total_pages):
        ...
        for name in soup.findAll("div", {"class": "info"}):
            ...
            try:
                ...
                info_dict.update({u'link': housetitle.a.get('href')})
                info_dict.update({u'houseID': housetitle.a.get('key')})
                houseinfo = housetitle.get_text().decode('unicode_escape').strip().split(' ')
                ...
                info_dict.update({u'years': years.get_text().decode('unicode_escape').strip()})

                community = name.find("span", {"class": "cj-text"})
                info_dict.update({u'community': community.get_text().decode('unicode_escape').strip()})
                ...
                info_dict.update({u'source': source.get_text().decode('unicode_escape').strip()})

                dealdate = name.find(
                    "div", {"class": "info-col deal-item main strong-num"})
                info_dict.update(
                    {u'dealdate': dealdate.get_text().decode('unicode_escape').strip().replace('.', '-')})
    ...
    for page in range(total_pages):
        ...
        for name in nameList:  # Per house loop
            ...
            try:
                ...
                title = communitytitle.get_text().decode('unicode_escape').strip('\n')
                ...
                info_dict.update({u'district': district.get_text().decode('unicode_escape')})

                cons = name.find("div", {"class": "con"})
                bizcircle = cons.findAll("a")
                info_dict.update(
                    {u'bizcircle': bizcircle[1].get_text().decode('unicode_escape').strip()})

                try:
                    tagList = name.find("span", {"class": "fang-subway-ex"})
                    info_dict.update({u'tagList': tagList.get_text().decode('unicode_escape').strip()})
                except:
                    info_dict.update({u'tagList': ''})

                onsale = name.find("span", {"class": "num"})
                info_dict.update({u'onsale': onsale.get_text().decode('unicode_escape').strip()})

                price = name.find("div", {"class": "price"})
                info_dict.update({u'price': price.span.get_text().decode('unicode_escape').strip()})
    ...
    for page in range(total_pages):
        ...
        for name in nameList:
            ...
            try:
                ...
                info_dict.update({u'region': wheres[0].get_text().decode('unicode_escape').strip()})
                info_dict.update({u'zone': wheres[1].get_text().decode('unicode_escape').strip()})
                info_dict.update({u'meters': wheres[2].get_text().decode('unicode_escape').strip()})

                other = name.find("div", {"class": "con"})
                info_dict.update({u'other': "".join(other.get_text().decode('unicode_escape').split())})
                ...
                price = name.find("div", {"class": "price"})
                info_dict.update(
                    {u'price': int(price.span.get_text().decode('unicode_escape').strip())})

                pricepre = name.find("div", {"class": "price-pre"})
                info_dict.update(
                    {u'pricepre': "".join(pricepre.get_text().decode('unicode_escape').split())})
...
def get_communityinfo_by_url(url):
    ...
    for info in communityinfos:
        try:
            ...
            res.update({'housetype': housetype.get_text().decode('unicode_escape').strip()})
            res.update({'year': year.get_text().decode('unicode_escape').strip()})
            res.update({'cost': cost.get_text().decode('unicode_escape').strip()})
